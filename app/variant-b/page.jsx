...
const FAQ = [
  ...
  ["Do you serve North America locally?", `Yes. ${CONTACT.companyShort} is based in ${CONTACT.cityState} and provides North American service — from spec and sourcing through delivery and support.`],
    ...
    <section id="about" className="border-t border-white/5 bg-ink-900 py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid items-center gap-12 lg:grid-cols-2">
          <Reveal>
            <span className="text-sm font-semibold uppercase tracking-[0.25em] text-electric-cyan">About {CONTACT.companyShort}</span>
            ...
            <p className="mt-5 text-base leading-relaxed text-white/65">
              {`${CONTACT.companyLegal} is an ${CONTACT.cityState}–based distributor of advanced LED hardware, serving architects, high-end retail brands, and system integrators across North America. Backed by a STEM engineering background and a global supply chain.`}
            </p>
            <dl className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
              {[[CONTACT.cityState, "HQ"], ["North America", "Service"], ["STEM-led", "Engineering"], ["Global", "Supply"]].map(([v, k]) => (
        ...
        <div className="grid gap-10 lg:grid-cols-5">
          <Reveal className="lg:col-span-2">
            ...
            <ul className="mt-8 space-y-4">
              {[
                [MailIcon, "Email", CONTACT.email, CONTACT.emailHref],
                [PhoneIcon, "Phone", CONTACT.phone, CONTACT.phoneHref],
                [PinIcon, "Headquarters", CONTACT.address, null],
        ...
        <div className="grid gap-10 md:grid-cols-3">
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3 text-sm">
              <li><a href={CONTACT.emailHref} className="inline-flex items-center gap-2 text-white/55 hover:text-electric-cyan"><span className="h-4 w-4"><MailIcon /></span> {CONTACT.email}</a></li>
              <li><a href={CONTACT.phoneHref} className="inline-flex items-center gap-2 text-white/55 hover:text-electric-cyan"><span className="h-4 w-4"><PhoneIcon /></span> {CONTACT.phone}</a></li>
              <li className="text-white/55">{CONTACT.address}</li>
            </ul>
          </div>
        </div>
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-6 sm:flex-row">
          <p className="text-xs text-white/40">{`© 2025 ${CONTACT.companyLegal} All rights reserved.`}</p>
          <p className="text-xs text-white/40">{`${CONTACT.cityState} · Serving North America`}</p>
        </div>
      </div>
    </footer>
  );
